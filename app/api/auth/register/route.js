...
function serializeCookie(c){return [`${c.name}=${encodeURIComponent(c.value)}`,`Max-Age=${c.maxAge}`,`Path=${c.path}`,`SameSite=${c.sameSite}`,c.httpOnly?"HttpOnly":"",c.secure?"Secure":""].filter(Boolean).join("; ");}
export async function POST(request){
 try{
  ...
  const response=Response.json({user:r.rows[0]},{status:201});
  response.headers.set("Set-Cookie",serializeCookie(sessionCookie(signUser(r.rows[0]))));
  return response;
